# Pre-Study Guide: Getting Ready for AI Engineering

Welcome! This guide will help you prepare for the AI Engineering course. **Good news: you don't need any programming experience for the first two weeks!**

---

## Course Philosophy

### Lowered Barriers for Everyone

This course is designed to be accessible to everyone, including non-technical students. We've structured it so that:

- **Weeks 1-2 require zero programming** — focus on concepts and using AI tools
- **Week 3 is when programming begins** — we'll teach you step by step
- **"Learn AI with AI"** — AI tools will help bridge any knowledge gaps

You'll be surprised how much you can accomplish with AI assistance, even as a beginner.

---

## Preparation by Week

### Before Week 1 (No Programming Required)

Week 1 is all about understanding AI concepts and learning to use AI tools. No coding needed!

**What to do:**
- [ ] Register for a ChatGPT account (free version is fine) at [chat.openai.com](https://chat.openai.com)
- [ ] Register for a Claude account (free version is fine) at [claude.ai](https://claude.ai)
- [ ] Ensure basic computer literacy (using a browser, downloading files, creating folders)
- [ ] (Optional) Install [Cursor IDE](https://www.cursor.com) — an AI-powered code editor

**What you'll learn in Week 1:**
- How to effectively prompt AI models
- Basic AI concepts and terminology
- Using AI as a learning companion

### Before Week 2 (No Programming Required)

Week 2 continues building your AI tool fluency. Still no programming required!

**What to do:**
- [ ] Get comfortable with the AI tools from Week 1
- [ ] Practice asking questions and following up with AI assistants
- [ ] Install [VS Code](https://code.visualstudio.com) or [Cursor](https://www.cursor.com) if you haven't already

**What you'll learn in Week 2:**
- Working with AI for research and analysis
- Understanding AI capabilities and limitations
- Preparing for the hands-on work ahead

### Before Week 3 (Programming Begins)

Week 3 is when we start coding! Here's how to prepare:

**What to do:**
- [ ] Install [Anaconda](https://www.anaconda.com/download) (we'll also teach this in class)
- [ ] Familiarize yourself with basic Python concepts (we'll use AI to help you learn)
- [ ] Set up a comfortable workspace with your computer
- [ ] (Optional) Ask an AI assistant to explain basic Python syntax

**Remember:** You don't need to be a Python expert. We'll teach as we go, and AI will help fill in the gaps.

---

## Self-Learn Resources (Optional)

The following chapters are available for students who want to get ahead. These are **completely optional** — we'll cover everything you need in class.

| Self-Learn Chapter | What's Inside | When to Reference |
|---|---|---|
| [Chapter 1: Tool Preparation](self_learn/Chapters/1/Chapter1.md) | Setting up development tools | If you want to explore tools early |
| [Chapter 2: Python and Environment Management](self_learn/Chapters/2/Chapter2.md) | Python basics, conda, virtual environments | Before Week 3 if you want a head start |
| [Chapter 3: AI Engineering Fundamentals](self_learn/Chapters/3/Chapter3.md) | Tokens, prompts, structured outputs | Weeks 3-4 if you're curious |
| [Chapter 4: Hugging Face Platform and Local Inference](self_learn/Chapters/4/Chapter4.md) | Running models locally | Weeks 5-6 for deeper dive |
| [Chapter 5: Resource Monitoring and Containerization](self_learn/Chapters/5/Chapter5.md) | Docker, monitoring, best practices | Throughout the course as needed |

**Feel free to skip these entirely** — we'll guide you to the right resources at the right time.

---

## What You DON'T Need to Know

To reduce anxiety, here's a list of things that are **NOT prerequisites** for this course:

- **Prior programming experience** — We'll teach Python from the ground up
- **Advanced mathematics** — Basic arithmetic is sufficient; AI handles the complex math
- **Git/GitHub** — Helpful but not required; we'll introduce it gently when needed
- **Linux/command line expertise** — We'll show you what you need to know
- **Machine learning theory** — We'll learn by doing, not by memorizing formulas
- **Expensive hardware** — Cloud resources and free tiers are sufficient
- **Computer science degree** — This course is designed for learners from all backgrounds

**What you DO need:**
- Curiosity and willingness to learn
- A computer with internet access
- Enthusiasm for AI technology

---

## Getting Help

### Before the Course Starts

Have questions before we begin? Here's how to get help:

1. **Ask an AI assistant** — ChatGPT, Claude, or the AI in Cursor can answer most setup questions
2. **Check the course forum** — Other students may have asked the same question
3. **Email the instructor** — For questions that AI can't answer

### Tips for Asking Questions

When asking for help (from AI or humans):

- Be specific about what you're trying to do
- Share any error messages you see
- Mention what you've already tried
- Don't worry about asking "dumb questions" — there are none!

### Using AI for Pre-Course Questions

You can ask AI assistants like:

> "How do I install Anaconda on Windows?"
> "What's the difference between VS Code and Cursor?"
> "Explain Python like I'm completely new to programming"

**Pro tip:** If the AI's explanation is too complex, just say "explain it simpler" or "explain like I'm 10 years old."

---

## Quick Checklist

Before the first class, aim to complete:

- [ ] ChatGPT account created
- [ ] Claude account created  
- [ ] VS Code or Cursor installed (optional for Week 1)
- [ ] Excitement level: high! 🚀

That's it! You're ready to start learning AI Engineering.

---

*Remember: Every expert was once a beginner. The AI tools you'll learn to use will accelerate your journey more than you can imagine.*
